...
impl CallLogger for CallLogInfra {
    ...
    fn set_logging_is_on(&mut self, is_on: bool) {
        self.logging_is_on.pop(); // TODO: Consider pop -> clear.
        self.logging_is_on.push(is_on);
    }
...
///
/// To visually separate the fragments logged by different threads
...
/// are indented by 1/4, 1/2, and 3/4 of the log width. Or in other words the _indent step_ is
...
struct ThreadIndents {
    ...
    /// The vacvant indents are reused starting with the lowest index,
    /// proportional to the lowest indent.
    indents_taken: Vec<bool>,
    /// The thread indent step - a string of white-spaces.
    ///
    /// The particular thread's indent is generated by using this thread indent `index` times.
...
impl ThreadIndents {
    ...
    /// Returns the tuple
    /// * of `index` in the indent storage
    /// * and the thread indent.
    ...
    /// Vacates the `index` in the indent storage (and the thread indent)
...
impl CallLoggerArbiter {
    ...
    pub fn remove_thread_logger(&mut self) {
        ...
        if let Some((_logger, thread_indent_id)) = self.get_thread_logger(current_thread_id) {
            // TODO: Consider `if let &mut Some` or `if let Some(&mut (` to avoid the line below.
