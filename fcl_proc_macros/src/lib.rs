        ...
        let output = {
            if let Ok(item) = syn::parse::<syn::Item>(attributed_item.clone()) {
                ...
                let closure_w_opt_comma =
                    syn::parse_macro_input!(attributed_item as crate::common::ExprClosureWOptComma); // Handles the compilation errors appropriately.
...
pub fn loggable_block_contents(
    ...
    let attr_args = syn::parse_macro_input!(attr_args_ts as crate::common::AttrArgs); // NOTE: Handles the compilation errors appropriately (checked).
...
mod common {
    ...
    ///   `false` otherwise.
    pub(crate) fn updated_loggable_attr_args(
    ...
    /// );
    /// ```
    pub(crate) fn remove_spaces(s: &str) -> String {
    ...
    pub(crate) struct ExprClosureWOptComma {
        pub(crate) closure: syn::ExprClosure,
    ...
    pub(crate) struct AttrArgs {
        pub(crate) prefix: proc_macro2::TokenStream,
        ...
        /// ```
        pub(crate) params_logging: ParamsLogging,
        ...
        /// }
        /// ```
        pub(crate) log_closure_coords: bool,
    }
