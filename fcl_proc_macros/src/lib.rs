...
pub fn loggable(_attr_args: TokenStream, _attributed_item: TokenStream) -> TokenStream {
    ...
    //     quote! {
    //         fn failed () {
    //             //call_logger!(failed); // The `FunctionLogger` instance.
    //         }
    //     }
    //     .into()
    }
...
fn quote_as_itemfn(func: ItemFn, attr_args: &Option<AttrArgs>) -> TokenStream {
    ...
    let output = quote! {
        #(#attrs)*
        #vis #signature {
            call_logger!(#func_name #generics); // The `FunctionLogger` instance. // TODO: Consider: `#func_name #generics` -> `#func_name#generics` (remove space)
            #block
        }
        // $( #[$meta] )*
        // $vis fn $name ( $( $arg_name : $arg_ty ),* ) $( -> $ret_ty )? {
        //     call_logger!($name); // The `FunctionLogger` instance.
...
fn quote_as_implitemfn(func: ImplItemFn, attr_args: &Option<AttrArgs>) -> TokenStream {
    ...
    let output = quote! {
        ...
        #vis #defaultness #signature {
            call_logger!(#func_name); // The `FunctionLogger` instance. TODO: What about `#generics`?
            #block
        }
    };
    output.into()
